...
serve(async (req) => {
  ...
  for (const sql of tables) {
    try {
      ...
      results.push({ ok: false, sql: sql.slice(0, 50), error: e.message });
    }
  }
  return new Response(JSON.stringify({ done: true, results }), { headers: { ...cors, "Content-Type": "application/json" } });
});
