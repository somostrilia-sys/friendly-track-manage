...
import { servicosIniciais } from "@/data/mock-data";
...
const TecnicoLink = () => {
  const { id } = useParams();
  const servico = servicosIniciais.find(s => s.id === id);
  const [aceito, setAceito] = useState(servico?.status !== "agendado");
  ...
  const compartilharGPS = () => {
    ...
    toast.success("GPS compartilhado! O cliente pode acompanhar sua localização.");
  };
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8 max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm">Painel do Técnico — {servico.id}</p>
      </div>
      ...
      <Card className="p-6 card-shadow space-y-5">
        <div className="flex items-center gap-3">
          ...
          {aceito && <Badge variant="default">Aceito</Badge>}
        </div>

        <div className="space-y-3 text-sm">
          <div><span className="text-muted-foreground">Cliente</span><p className="font-medium text-lg">{servico.clienteNome}</p></div>
          ...
          <div><span className="text-muted-foreground">Valor</span><p className="font-semibold text-lg text-primary">R$ {servico.valorServico}</p></div>
        </div>
