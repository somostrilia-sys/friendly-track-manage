...
import { tarefasIniciais, Tarefa } from "@/data/mock-data";
...
const Tarefas = () => {
  ...
  const [form, setForm] = useState({ titulo: "", descricao: "", prioridade: "media" as Tarefa["prioridade"], responsavel: "", dataLimite: "" });
  ...
  const toggleConcluida = (id: string) => {
    setTarefas(prev => prev.map(t => t.id === id ? { ...t, status: t.status === "concluida" ? "pendente" : "concluida" } : t));
  };
  ...
  const salvar = () => {
    ...
    setModalOpen(false);
    setForm({ titulo: "", descricao: "", prioridade: "media", responsavel: "", dataLimite: "" });
          ...
          return (
            <Card key={t.id} className={`p-4 card-shadow transition-shadow ${t.status === "concluida" ? "opacity-60" : "hover:shadow-md"}`}>
              <div className="flex items-start gap-3">
                <Checkbox checked={t.status === "concluida"} onCheckedChange={() => toggleConcluida(t.id)} className="mt-1" />
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Prioridade</Label>
                <Select value={form.prioridade} onValueChange={v => setForm(f => ({ ...f, prioridade: v as Tarefa["prioridade"] }))}>
            ...
            <div><Label>Data Limite</Label><Input type="date" value={form.dataLimite} onChange={e => setForm(f => ({ ...f, dataLimite: e.target.value }))} /></div>
          </div>
          <DialogFooter>
